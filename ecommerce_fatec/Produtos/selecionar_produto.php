...
include_once "../Funcoes/BancoDeDados.php";
        ...
        $conexao = conectar(); 
        $sql = "select nome_arquivo, produto.codigo_prod, produto.nome_pro, codigo_img from imagem inner join produto on imagem.codigo_prod = produto.codigo_prod where quantidade > 0 group by codigo_prod order by produto.nome_pro ASC"; 
      ...
      <div class="row">
                ...
                <?php 
                  foreach ($resultado as $key => $registro) {
                    $id = $key;
                    $cod = $registro["codigo_prod"];
                    $nome = $registro["nome_pro"];
                    $img = $registro["nome_arquivo"];
                    $cod_img = $registro["codigo_img"];
    
                    echo "<div class='col-lg-3 col-md-6'>
                          <div class='single-feature'>
                          <img src='$img' width='172' height='169' >
                          <p>$nome</p>
                          <p>Código: $cod</p>
                          <p><a href='editar_produto.php?cod=$cod' class='genric-btn primary small info'>Editar/Excluir Informações</a></p>
                          <p><a href='editar_img.php?cod=$cod&cod_img=$cod_img&img=$img' class='genric-btn primary small info'>Inserir/Excluir Imagens</a></p>
                          </div>
                          </div>";
                  }
                      ?>
